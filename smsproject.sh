...
search_student_record()
{
  ...
  echo "What student do you want to search based on ID or name."
  read -r ans

  if grep -q "$ans" "$fileName"
  then
    grep "$ans" "$fileName"
  else
    echo "Student record not found. Check if student ID or full name is correct."
  fi
